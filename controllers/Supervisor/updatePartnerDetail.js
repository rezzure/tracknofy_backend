...
//         if (partner.createdBy.toString() !== supervisor._id.toString()) {
//             partner.createdBy = supervisor._id;
//             partner.createdByModel = 'Supervisor'; // Also update the model reference
//         }
...
const updatePartnerDetail = async (req, res) => {
    ...
    const { partnerType, partnerName, partnerMobile, partnerAddress, email, dailyWagesAmount, rate, fullWorkRate } = req.body;
    console.log(partnerType, partnerName, partnerMobile, partnerAddress, email, dailyWagesAmount, rate, fullWorkRate);
    ...
    try {
        ...
        // Update new fields with proper type conversion
        if (dailyWagesAmount !== undefined) partner.dailyWagesAmount = dailyWagesAmount ? Number(dailyWagesAmount) : null;
        if (rate !== undefined) partner.rate = rate ? Number(rate) : null;
        if (fullWorkRate !== undefined) partner.fullWorkRate = fullWorkRate ? Number(fullWorkRate) : null;
